...
このテンプレートは `terraform-gcp` profileを宣言し、foundationの9個のcheckに、常に結果を
返す `iac-scan` contextを追加します。repository policyはfamily checkを重複して保持せず、
そのrepository固有の運用overrideだけを保持します。
